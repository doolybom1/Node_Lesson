...
// router를 선언하는 곳에서 (app.js에서) 매개변수로 config 값을 전달하기 위해서
...
module.exports = function(app,config){
    ...
    */
    ...
    // express framework에서pagination을 구현할 객체 선언하여 사용할 수 있도록 초기화
    // 기본 limit 10 , 최대 limit 50개로 설정
    app.use(paginate.middleware(10,50))
    const pageLimit = 10

    // 이하의 router 메서드를 실행하기전에 먼저 실행하여 변수, 세팅등을 설정한 후
    ...
    router.get('/searchStation',function(req,res){
        ...
        /*
            ...
            {칼럼명 : RegExp(값, 'ig')} : 중간문자열검사 SQL에서 %값%
        */
    ...
    router.get('/viewStation', function (req, res) {
        ...
        /*
            현재 선택된 페이지 번호가 req.query.page에 담겨서 절달되어 오면 그 페이지 번호에
            pageLimit값을 곱하여 몇번째 데이터부터 읽어 올것인지를 정하는 값을 계산
            offset == skip
        */
        let offset = (req.query.page - 1) * pageLimit
        gjStation.count({})
        .exec(function(err,count){
            //전체 데이터가 구해지면 한페이지 데이터 개수로 나누어 총 몇페이지가 필요한지를 계산한다
            let pageCount = Math.ceil(count / pageLimit)

            // 한페이지에 보일 데이터(pageLimit)
            // 전체 페이지 개수(pageCount)
            // 현재 선택된 페이지(req.query.page : 페이지에서 클릭한 페이지 번호)
            // 이 3가지의 데이터를 paginate의 getArrayPages 메서드한테 전달하면 페이지를 그리는데 필요한 정보가 담긴
            // pageArray객체를 자동으로 생성해 준다
            let pageArray = paginate.getArrayPages(req)(
                pageLimit, pageCount, req.query.page
            )
            // 실제 한페이지에 보여줄 데이터를 읽어오는데 위에서 계산한 pageLimit, offset값을 활용하여
            // 필요한 데이터만 추출한다
            gjStation.find({})
            .limit(pageLimit)
            .skip(offset)
            .exec(function(err,stations){
                // 데이터 리스트 페이지에 지금까지 생성한 값들을 전달하여 데이터리스트를 보이고 
                // 하단에 페이지 리스트를 보여서 pagination을 구현하다
    ...
    router.get('/getStation',function(req,res){
        ...
        request({
            ...
            if(resultJson.RESULT.RESULT_CODE == 'SUCCESS'){
                // 데이터가 정상 수신
                // 만약 기존 데이터가 있으면 모두 삭제를 하고 새로운 데이터로 대체
                gjStation.deleteMany(function(){
                    ...
                    gjStation.collection.insertMany(station_list,function(err,result){
                        if(err){
                            res.send('data bulk inser error')
                        }else{
                           //res.json(result)
