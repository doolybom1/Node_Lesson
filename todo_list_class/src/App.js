...
class App extends Component {
  ...
  */
  ...
  // == : equal 연산자, eq 연산을 할때 자동으로 형변환을 수행, 어떤 경우에는 전혀 예상치 못한 true를 나타내기도 한다
  // === : identity 연산자
  handleKeyPress = e => {
    if (e.key == "Enter") {
      this.handleCreate();
  ...
  // 현재 클릭된 아이템(id)만 남기고 나머지 리스트만 추출하기
  handleDelete = (id)=>{
    const {todoList} = this.state
    this.setState({
  ...
  render() {
    ...
    const { handleCreate, handleChange, handleKeyPress, handleToggle, handleDelete } = this;
